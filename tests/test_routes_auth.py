...
def test_login_with_missing_fields(client):
    res = client.post('/login', data={'username': '', 'password': ''}, follow_redirects=True)
    assert b"Username and password are required" in res.data
    assert res.status_code == 200

def test_login_with_invalid_credentials(client):
    res = client.post('/login', data={'username': 'notarealuser', 'password': 'wrongpass'}, follow_redirects=True)
